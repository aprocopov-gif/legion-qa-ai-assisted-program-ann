...
async function listApPrograms(page: Page): Promise<ProgramRow[]> {
  ...
  await page.getByRole("table").waitFor({ state: "visible", timeout: 15_000 }).catch(() => {});
  ...
  for (let i = 0; i < count; i++) {
    ...
    const description =
      (await descP.count()) > 0 ? ((await descP.textContent()) ?? "").trim() : "";
...
function printTable(programs: ProgramRow[]) {
  ...
  const nameW = Math.max(12, ...programs.map((p) => p.name.length), "Program Name".length);
  const descW = Math.max(11, ...programs.map((p) => p.description.length), "Description".length);
