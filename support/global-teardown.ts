import { deleteProgram, fetchAllPrograms, getDidaxisConfig } from "../.agents/skills/didaxis-program-deleter/support/delete-program";
...
async function globalTeardown() {
  ...
  const targets = programs.filter((program) => (program.name ?? "").startsWith("AP_"));
  ...
  console.log(`[global-teardown] AP_ cleanup: tracked=${targets.length}, cleaned=${cleaned}, failed=${failed.length}`);
  if (failed.length > 0) {
