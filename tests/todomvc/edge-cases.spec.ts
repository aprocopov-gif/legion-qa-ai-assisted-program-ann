...
test.describe("Edge cases", () => {
  ...
  test("TC-014 (AC2 edge) — Leading/trailing whitespace in a new item is trimmed", { tag: "@medium" }, async () => {
  ...
  test("TC-015 (AC2 edge) — Internal multi-space sequences are preserved verbatim", { tag: "@low" }, async () => {
  ...
  test("TC-016 (AC2 edge) — Special characters and emoji are accepted as plain text (no XSS)", { tag: "@high" }, async ({ page }) => {
  ...
  test("TC-017 (AC2 edge) — Very long title (1000 chars) is preserved without breaking layout", { tag: "@medium" }, async () => {
  ...
  test("TC-018 (AC2 edge) — Duplicate titles are stored as independent items", { tag: "@medium" }, async () => {
    await app.addTodo("Buy milk");
    await app.addTodo("Buy milk");
  ...
  test("TC-019 (AC2 boundary) — Adding 4 items in rapid succession produces exactly 4 entries", { tag: "@medium" }, async ({ page }) => {
  ...
  test("TC-020 (AC4 boundary) — Removing all 4 items one by one returns the app to empty state", { tag: "@high" }, async ({ page }) => {
    ...
    for (const t of titles) {
      ...
      if (expected > 0) {
        ...
        await expect(app.counter).toHaveText(`${expected} ${expected === 1 ? "item" : "items"} left`);
      }
    }
  ...
  test("TC-021 (AC3 edge) — Toggle-all marks every item finished; counter shows 0 left", { tag: "@medium" }, async () => {
    const titles = ["Buy milk", "Walk the dog", "Write tests", "Read book"];
    await app.addMany(titles);
    await expect(app.counter).toHaveText("4 items left");
  ...
  test("TC-022 (AC3 edge) — A finished item appears under /completed filter and is hidden under /active filter", { tag: "@medium" }, async () => {
