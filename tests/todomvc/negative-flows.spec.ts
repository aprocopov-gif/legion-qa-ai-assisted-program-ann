...
test.describe("Negative flows", () => {
  ...
  test("TC-009 (AC2 negative) — Empty input + Enter does NOT create a todo", { tag: "@high" }, async () => {
  ...
  test("TC-010 (AC2 negative) — Whitespace-only input does NOT create a todo", { tag: "@high" }, async () => {
    await app.addTodo("   ");
  ...
  test("TC-011 (AC3 negative) — Toggling a finished item back unfinishes it", { tag: "@medium" }, async () => {
  ...
  test("TC-012 (AC4 negative) — Removing one item must NOT remove other items", { tag: "@high" }, async () => {
  ...
  test("TC-013 (AC3 + AC4 negative) — Finished+removed item does not leak completed state to remaining items", { tag: "@medium" }, async () => {
