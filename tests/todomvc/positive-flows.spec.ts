...
test.describe("Positive flows", () => {
  ...
  test("TC-001 (AC1) — Empty TODO list is ready to receive items on first load", { tag: "@high" }, async ({ page }) => {
  ...
  test("TC-002 (AC2) — User adds a single item by pressing Enter", { tag: "@high" }, async () => {
  ...
  test("TC-003 (AC2) — User adds exactly 4 items in insertion order", { tag: "@high" }, async () => {
  ...
  test("TC-004 (AC3) — Marking an item as finished updates UI, counter, and storage", { tag: "@high" }, async () => {
    const titles = ["Buy milk", "Walk the dog", "Write tests", "Read book"];
    await app.addMany(titles);
    await expect(app.counter).toHaveText("4 items left");
    ...
    const stored = await app.readStorage();
    const byTitle = (t: string) => stored!.find((s) => s.title === t)!.completed;
  ...
  test("TC-005 (AC3) — A finished item stays finished across reload", { tag: "@medium" }, async ({ page }) => {
    const titles = ["Buy milk", "Walk the dog", "Write tests", "Read book"];
    await app.addMany(titles);
    await app.toggle("Buy milk");
  ...
  test("TC-006 (AC4) — User removes a single item via the × destroy button", { tag: "@high" }, async () => {
  ...
  test("TC-007 (AC4) — Removing the last item resets the UI to empty state", { tag: "@high" }, async ({ page }) => {
    await app.addTodo("Buy milk");
  ...
  test("TC-008 (AC2 + AC4) — Removing one item from a list of 4 leaves 3 in original order", { tag: "@high" }, async () => {
    await app.addMany(["Buy milk", "Walk the dog", "Write tests", "Read book"]);
    const before = await app.readStorage();
    ...
    expect(await app.titles()).toEqual(["Buy milk", "Write tests", "Read book"]);
    await expect(app.counter).toHaveText("3 items left");
